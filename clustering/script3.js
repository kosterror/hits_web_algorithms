...
animation({
    ...
    render() {
        new_point = new Point(mouse.x, mouse.y, 0);

        if (mouse.draw && isCanAddpoint(new_point)) {
            data_points[data_points.length] = new_point;
...
function isCanAddpoint(point) {
    if (point.x > POINT_RADIUS && point.x < WIDTH - POINT_RADIUS && point.y > POINT_RADIUS && point.y < HEIGHT - POINT_RADIUS) {
        let index = getNearestPointIndex(point);

        if (index == -1) {
            return true;
        } else if (Math.sqrt(DistanceSquared(data_points[index], point)) > POINT_RADIUS) {
            return true;
        }
    }
    return false;
...
function getNearestPointIndex(p) {
    let minDistance = -1;
    let index = -1;

    for (let i = 0; i < data_points.length; i++) {
        let distance = Math.sqrt(DistanceSquared(data_points[i], p));
